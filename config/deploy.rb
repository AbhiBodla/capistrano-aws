server '18.118.15.217', port: 22, roles: [:web, :app, :db], primary: true
...
set :deploy_to,      "/home/#{fetch(:user)}/apps/#{fetch(:appliocation)}"
set :puma_blind,     "unix://#{shared_path}/tmp/sockets/#{fetch(:application)}-puma.sock"
...
set :puma_init_active_record, true #change to false when not in using ActiveRecord
...
namespace :deploy do
  ...
  task :check_revision do
   ...
   task :initial do
     on roles(:app) do
        before 'deploy:restsrt', 'puma:start'
        invoke 'puma:restart'
     end
    end
